...
* 메모리 관리 자동화를 위한 기능
* 프로그램 실행 중 더 이상 사용되지 않는 개체를 자동으로 찾아 제거함으로써 메모리 누수를 방지
* 개발자가 메모리를 명시적으로 해제할 필요가 없도록 도와준다
*
* 명시적 : 분명히 드러나는 것
*
* 가비지 컬렉션 동작 : 자바가 특정 기능과 기준을 통해 메모리를 정리하는 것
*                      메모리 정리를 직접적으로 언제 진행해달라 설정할 수 있음
*                      ex. System.gc(); // 메모리 정리 실행 요청
*
*
...
public class GarbageCollectionEx {
    public static void main(String[] args) {
        ...
        // String 의 경우 불변성으로 인해 적합한 예제는 아니지만 가장 직관적으로 가비지 컬력션을 확인할 수 있어 예제 작성
